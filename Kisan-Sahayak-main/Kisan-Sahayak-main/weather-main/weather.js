// const apikey="7cce97486d4880319841d0d65f77eb90";
...
async function searchByCity() {
    ...
    try {
        const response = await fetch(`http://localhost:4567/weather?city=${input}`);
        if (!response.ok) {
...
function updateWeatherUI(data) {
    ...
    // Update icon (requires OpenWeatherMap icons)
    const iconCode = data.weather[0].icon;
    document.getElementById('img').src = `http://openweathermap.org/img/wn/${iconCode}@2x.png`;
}
